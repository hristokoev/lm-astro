...
export default function FormButtons({ index, loading }) {
  function handlePrev(index) {
    if (formData.value.rooms === "Office") {
      if (index === 3) formJump(1);
			else if (index === 6) formJump(4);
			else formPrev(index)
    } else {
			formPrev(index)
		}
  ...
  return (
    <div className="max-w-xs flex flex-col-reverse sm:flex-row mx-auto sm:max-w-none sm:inline-flex sm:justify-center gap-y-4 sm:gap-y-0 sm:gap-x-4 select-none">
      {index > 0 && (
        <div>
          <button
            className="btn text-slate-700 bg-white hover:bg-slate-100 w-full group"
            type="button"
      ...
      <div>
        {index < 12 ? (
          <button
            className="btn text-white bg-slate-700 hover:bg-slate-600 border-slate-600 w-full"
            type="submit"
          >
          ...
          <button
            className={`btn text-white bg-malibu-500 hover:bg-malibu-600 border-malibu-600 w-full min-w-[180px]`}
            type="submit"
            disabled={loading}
          >
            {loading ? (
